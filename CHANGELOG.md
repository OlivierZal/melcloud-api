...
- **A group read no longer overwrites the members' snapshots.** `ClassicBuildingFacade`, `ClassicFloorFacade` and `ClassicAreaFacade` `getGroup()` patched every ATA member's registry model with the non-null fields of the group state `/Group/Get` answers — the group's `OperationMode`, `Power`, `SetTemperature` and vanes — as if the read had been a write. That state is MELCloud's state for the GROUP, not what each member reports, and a member's snapshot is the body of its next write: a unit in Cool whose zone reads Auto carried `OperationMode: Auto` in its next temperature write, unflagged (so the flags doctrine says the unit ignores it), then echoed back into the registry by the write's own decorator, so the consumer's tile showed Auto until the next sync — and MELCloud's own display, which holds the posted body until the unit reports back, showed it too. com.melcloud's ATA group widget calls this read on load, on Refresh and on every real-time device update, so a widget kept open on a dashboard re-patched every member after every sync. Consistent with the field report of 2026-09-24 ("the app repeatedly changes the operating mode from Cool back to Auto", no Flow involved), whose log was not available to confirm it: the mechanism explains a displayed Auto with certainty, and a real mode change only if MELCloud applies the unflagged fields of a body. The read now returns the group state and touches nothing; the write path (`updateGroupState`, whose decorator propagates the payload it actually posted) is unchanged.

## [59.2.0] - 2026-09-22

### Changed

- **The Home `/context` drift is one event in the log, not one line per fetch.** While the strict schema refuses the payload, the salvage keeps every unit that still validates — and used to log the drift on every fetch — 1,440 lines a day at Home's one-minute default cadence, each carrying the whole ZodError. The drift now rides `@olivierzal/api-core`'s `FailureStreaks` (public since 1.9.0; the exact pin advances to it), keyed on the refused paths: reported when it opens, when the paths change and at most every five minutes while it lasts, then closed by one line counting the drifting fetches when the strict parse holds again. heatzy-api 20.1.0 streaks its device reads on the same class — the mechanism is shared, the subjects are each SDK's own. `describeRefusedPaths` is exported from the validation module for it (`parseOrThrow` already computed it inline).
...
- **BREAKING — `HomeAtaDeviceData.connectedInterfaceType` is an open `string`, and the `HomeDeviceConnectedInterfaceType` alias is gone** (also as `DeviceConnectedInterfaceType` on the `/home` subpath). The field was the closed union `'fourthGenWifi' | 'melCloudWiFi'`, and `HomeAtaDeviceDataSchema` refused any other value; once opened, the alias would only have restated `string`. Nothing in the SDK or in com.melcloud reads the field, yet a unit on the next adapter family would have failed the strict `/context` schema: the salvage parse pruned it from the registry, and the drift line was logged on every fetch for as long as the family stayed unknown. heatzy-api 19.0.0 fixed the same shape on its wire, where it had already broken every Glow, Onyx, Shine, calibrated Pro and V1 radiator: a read checks the wire's types, and new vocabulary never breaks a consumer's sync. The two known values stay documented on the field. A consumer that imported the alias types the value as `string`.

### Fixed

- **A refused payload prints its issues once.** `parseOrThrow` interpolated the ZodError's message, the JSON of every issue, into its own message while also attaching that ZodError as `cause`, so a logged `ValidationError` printed the issue list twice. The message now names each failing path once (`Invalid API response shape (GET /context (salvage)): buildings.0.id`, `(root)` for the payload itself), and the issues stay in the `cause`. For a refused union it names the paths of the branch the payload came closest to, so a refused energy report names its `LabelType` rather than `(root)`. The `issue` of a `validation` `Result` (`ApiRequestError`) is that message, so it shrinks accordingly: from the JSON of every issue to the failing paths, the full detail still on its `cause`. No received value is named: the login, token and `/context` payloads carry credentials and personal data.
...
- **The exact `@olivierzal/api-core` pin advances to 1.7.1: the auto-sync tick is parked around every mutation.** A refresh that overlapped a write read the pre-write state back into the registry — and through `onSync` into a Homey capability, which flipped back for one cycle — and one that followed the write too closely read a unit that had not applied it yet (a Classic unit keeps only the flagged fields a few seconds after the POST). The core now holds the tick for the duration of any non-GET request and for a 3-second settle window after it. The hold only ever delays the planned tick, never advances it: the 5-minute cadence stays a 5-minute cadence, and a write costs no extra refresh — which matters under MELCloud's rate limits. Reads hold nothing. The `HttpClientConfig.describeFailure` reader that 1.7.x also adds is not seated here: MELCloud's refusals carry no reason in their body that the status line does not already say.

## [57.2.1] - 2026-09-14

### Changed

- **The exact `@olivierzal/api-core` pin advances to 1.6.0: two session-epilogue fixes, behaviour only.** A stored pair that the server has definitively refused is now parked until the next accepted sign-in — it was replayed every 15-minute backoff window for the life of the process, the hammering the disarm-on-refusal verdict exists to prevent; a throttle (MELCloud's `ErrorId` 6) keeps its retry, since it says nothing about the pair. And only a sign-in that began after the last sign-out claims the session: a stale flight answered after a sign-out no longer leaves a session standing behind it. No surface moves, so the adoption is the pin.
...
- **`fetchDevices` takes the refresh it runs as an explicit callback typed against the host**, instead of resolving `syncRegistry()`-else-`api.fetch()`-else-`TypeError` at runtime over a structural host that nothing type-checked. Each of the ten sites already had exactly one path (`HomeAPI` → `syncRegistry()`, the Classic facades → `api.fetch()`), so behaviour is identical; the three test clauses that existed only to exercise the dispatch over fabricated hosts go with it, and so does the stale docstring that named `registry.syncDevices()`.
...
- **The Classic write path returns to the shape it had before 56.0.0, and 56.0.0's premise is withdrawn.** `EffectiveFlags` is AUTHORITATIVE on `/Device/Set{Ata,Atw,Erv}`: the unit applies the flagged fields and lets every other field in the body fall back to its previous value a few seconds later. The probe that led to 56.0.0 read the endpoint's immediate response, which echoes the whole body it received, and concluded the opposite — that the flags were decorative and that omitted fields were zero-filled. Both readings are artefacts of reading the echo instead of re-reading the device. A Classic write can only be judged by re-reading the unit seconds afterwards.
  ...
  Migration: a Classic `updateValues` no longer throws `StateReadError` and no longer spends a second request; `NoChangesError` is judged against the last synced state, as it was through 55.x. A host that branched on `StateReadError` drops that branch. Note the cost of that last clause, which 56.0.0 had closed: a write back to a value the stale snapshot still holds is folded away silently, so up to one sync interval after a change made elsewhere, a Homey command can do nothing without saying so.
...
- **The `EffectiveFlags` verdict in `CLAUDE.md` is rewritten around the corrected mechanism**, with the method note that makes it falsifiable: judge a write by a delayed re-read, never by the POST's response. It also records what a stale snapshot really costs. Since the change set is filtered against the snapshot before the flags are computed, the flag set is always a subset of the fields the caller named: a stale snapshot cannot invent a flag, it SUPPRESSES one, and the write is folded away as `NoChangesError` while the unit keeps the value the user was trying to leave. The write that does re-impose a field is one whose caller names it — a form prefilled from the snapshot and applied after a sync moved it, which is the shape `updateGroupState` sends and the first hypothesis to test for the "vane resets to Auto" and "cooling reverts to 24°" reports (com.melcloud #1408).

## [56.0.0] - 2026-09-07

> Superseded by 57.0.0: the premise below — that `EffectiveFlags` is decorative and that omitted fields are zero-filled — was read from the endpoint's immediate response and is wrong.

### Breaking changes

- **Classic writes merge onto a LIVE read of the unit, never onto the last sync's snapshot.** Live-probed 2026-09-05 on `/Device/SetAta`: `EffectiveFlags` is decorative — a POST flagging `SetTemperature` alone still APPLIED the carried, unflagged `VaneVertical` and `OperationMode` — and a partial body is not the remedy, because an omitted field is ZERO-FILLED. Full-state posting is therefore mandatory, and the only thing a write can get wrong is the state it merges the delta onto. Until now that was the registry snapshot from the last sync (up to the 5-minute default interval old), so any write re-imposed every field another writer had changed since: the "vertical vane keeps resetting to Auto" and "cooling reverts to 24° every 10 minutes" (com.melcloud #1408) field reports are this one mechanism, on every Classic ATA and ATW field alike. `BaseDeviceFacade.updateValues` now reads the unit (`/Device/Get`) immediately before posting, catches the registry model up with the answer, and merges the delta onto THAT — one extra `GET` per DEVICE write: a zone facade's `updateGroupState` posts `/Group/SetAta` once and spends none, so only a per-member fan-out (`ClassicDeviceAtaFacade.updateGroupState`, one `updateValues` each) spends one per member. Two verdicts follow: a read that fails REFUSES the write with the new `StateReadError` (never a fallback to the snapshot — a rollback is worse than a retry), and `NoChangesError` is judged against the unit's live state, not the snapshot's — a write refused that way leaves the registry model caught up with the read but announces no sync (`@syncDevices` notifies a resolved write only), so the caught-up fields reach `onSyncComplete` observers with the next sync tick or `getValues`, as before. Kernel-pinned on the real Classic leg, wire to wire (`tests/contracts/classic-write-freshness.test.ts`). Home is unaffected: its PUT is a delta, live-verified the same day. What this cannot fix is IR-remote lag (the unit-to-MELCloud reporting delay).

  Migration: a Classic `updateValues` can newly throw `StateReadError` (`failure.kind` says whether to retry) and newly throws `NoChangesError` for a value the unit already holds even when the snapshot disagreed; budget one more request per write against the rate-limit window.

- **The four SDK-internal decorators leave the public surface.** `classicUpdateDevice`, `classicUpdateDevices`, `fetchDevices` and `syncDevices` were exported from the root barrel and aliased on `/classic` (`updateDevice`, `updateDevices`, `fetchDevices`, `syncDevices`) and `/home` (`fetchDevices`, `syncDevices`) with zero consumers in the family: they are wiring bound to SDK internals (`classicUpdateDevices` binds `this: ClassicZoneFacade`, `fetchDevices` resolves a host over `ClassicAPIAdapter`), and typedoc already hid their option types as "SDK-internal wiring consumers never call". `src/decorators/index.ts` stays the internal barrel; the `Decorators` docs category and the export-map exemption go with the exports. heatzy-api's twin export crosses in the same wave.

  Migration: none known — no repo imported them. A host that did applies its own decorator over the public facade methods instead.

### Added

- **`StateReadError`** — thrown by a Classic `updateValues` whose pre-write live read failed: `entityId` names the device, `failure` carries the read's classified `ApiRequestError` (the same variant `getValues` answers), and the read's underlying error rides `cause` when it has one. Exported from the root barrel and the `errors/` surface like its siblings.

### Changed

- **`@olivierzal/api-core` 1.3.0: three more twins and the test helpers cross to the core.** `ValidationError` is now the core's class re-exported under its existing name and path (the `RegistrySyncError` pattern — the class imports nothing from zod, so the coupling that keeps `parseOrThrow` here never applied to it; a host now classifies drift by one `instanceof` across both SDKs). The `syncDevices` decorator factory is the core's, generic over this SDK's `SyncParams` — call sites are unchanged, with one observable delta inside the SDK: a bare `@syncDevices()` forwards `undefined` to `notifySync` where the local copy forwarded `{ type: undefined }` (`onSyncComplete` observers already treated the payload as "registry changed, re-inspect"). The sign-in normalization is the core's protected `SessionAPI.toAuthFailure(error, message)`, reading the `authFailureStatuses` vocabulary the reactive-401 rung already owns (the default `[401]` here): `normalizeUnauthorized` is deleted and Home's `doAuthenticate` binds to it in two statements — `const authError = this.toAuthFailure(error, 'MELCloud rejected the credentials')`, `if (authError !== null) { throw authError }`, then a bare `throw error`; never a `throw … ?? error` one-liner, which the family's `only-throw-error` rule refuses (the `??` expression is typed `unknown`; a catch variable rethrown bare is admitted) — same wrapping, same message, one owner. And `tests/helpers.ts` keeps only what is this SDK's own (`okValue`, `matchObject`, `mockResponse`); everything else the three SDK suites carried as a hand-maintained twin (`cast`, `defined`, `mock`, the logger and setting-store doubles, the transport spy, `mockFetchResponse` — which now nulls the body on 204, 205 AND 304 —, the `HttpError` factories, the native-`Temporal` clock spies) comes from the new `@olivierzal/api-core/testing` subpath, proven in the core's own suite before a consumer inherits it. `createMockHttpClient(HttpClient, baseURL)` now takes THIS repo's transport class first, so the spy-wrapped client is the redaction-seating one the resolver accepts. The subpath imports `vitest` from this repo's devDependencies and is never re-exported by the core's root barrel, so production imports are untouched. With the helpers gone, so are the suite blocks that re-ran the core's own clauses against re-exported symbols: `errors.test.ts` and `type-helpers.test.ts` keep only clauses naming SDK classes or pinning the wiring (an SDK error extends the core's base and passes the family guard; a re-exported class IS the core's), `base-api.test.ts` drops the `normalizeUnauthorized` table, `decorators.test.ts` the `syncDevices` one — re-testing core behavior would let coverage be satisfied by the wrong suite.
...
- **`getEnergySeries` — the normalized Home energy-history read.** The Home telemetry wire dialect was half-owned: this SDK wrote the queries while consumers decoded the answers — com.melcloud parsed the nanosecond-padded UTC wall-clock stamps, knew the `interval` vocabulary from this repo's own documentation, and carried the per-type energy units (`Wh` vs `kWh`) in its report strategies. Both Home device facades now project `getEnergy`'s bundle onto a normalized series so that decode dies entirely: every sample carries `atEpochMs` (the UTC wall-clock stamp anchored as an epoch-ms instant) and `kilowattHours` (one documented unit whatever the device type — the ATW interval measures are kWh per bucket on the wire and cross unscaled; the ATA cumulative measure is watt-hours and is divided by 1000, the exact arithmetic consumers ran themselves, so adopted values are bit-identical). A sample the wire garbles degrades field-by-field to `null` — never `NaN`, which `JSON.stringify` silently rewrites, and never a throw — and keeps its entry. The query is `getEnergy`'s with `interval` narrowed to the wire's closed .NET vocabulary (`HomeEnergyInterval`: `Minute`/`Hour`/`Day`/`Week`/`Month`), and `measure` still selects the direction on ATW only. The wire-verbatim `getEnergy` deliberately stays: it is the escape hatch for consumers that need the raw `measureData` envelope (measure names, unscaled values), and removing it would be a breaking change this release has no cause for. New exports: `HomeEnergyInterval`, `HomeEnergySeriesPoint`, `HomeEnergySeriesQuery` (aliased `EnergyInterval`/`EnergySeriesPoint`/`EnergySeriesQuery` on the `/home` barrel).
- **`operationalState` on the Classic ATW facade.** The Home ATW facade has shipped the device-level derived state since 53.x while Classic's twin was projected app-side (com.melcloud's `operationModeStateFromDevice` table). The getter now sits beside its `hotWater.operationalState` and per-zone siblings, deriving the same cross-dialect vocabulary (`HomeAtwOperationalState` — the `Home` prefix is history, not a branch, like `ClassicGroupState`'s) from the wire's numeric `OperationMode` through the new total bijection `atwOperationalStateFromClassic` (exported from `/enum-mappings` like its zone-mode sibling). The six known states map exactly as the app displays them today; an out-of-vocabulary wire number reads `null` like the Home facade's unknown FTC strings, so new wire vocabulary can never break a consumer's sync. The cross-dialect ATW-state contract table gains the top-level clause on both legs.

### Changed

- **`@olivierzal/api-core` 1.2.0: the auto-sync failure lines now name their dialect.** The extraction crossed `SyncManager` receiving the RAW host logger where every other seat got the labelled one — a latent bug pinned AS IT WAS, its fix recorded for its own PR "after both adoptions land". Both adoptions landed 2026-09-04, the deferral expired, and api-core 1.2.0 hands the manager the labelled logger like every other seat: a rejected auto-sync tick now reports as `[Classic] Auto-sync failed: …` / `[Home] Auto-sync failed: …` in the diagnostic reports users paste into issues, so a host running both dialects can finally tell which account a line was about. The kernel's deliberate-asymmetry clause flips with the adoption and now holds the fix.
