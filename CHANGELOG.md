...
- **The legionella wire spelling is `LegionellaPrevention`** (live-captured during a running cycle, 2026-07-13): `HomeAtwOperationMode` replaces its guessed `Legionella` member, and both facade derivations (`operationalState`, `hotWaterOperationalState`) map the observed spelling — the bare `Legionella` stays as defensive tolerance. Before this, a legionella cycle read `null`/`idle`.
...
- **`HomeAtwOperationMode`** — the top-level FTC operation-mode vocabulary reported by the `OperationMode` device setting (live-captured `Cooling` and `Stop`, plus the states the MELCloud Home app surfaces), for consumers deriving an operational state. The facade's `operationMode` getter keeps returning `string` because firmware revisions may expose further values.
